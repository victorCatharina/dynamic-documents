...
describe('SubmissionValidationService', () => {
  let service: SubmissionValidationService;
  ...
  it('should strictly reject INTEGRATION fields when submitted via public form (allowIntegrationFields: false)', () => {
    const data = {
      nomeCliente: 'Carlos Alberto',
      codigoIntegracao: 'TENTATIVA_DE_INJECAO', // Should be rejected in public form!
    };
    ...
    expect(
      result.errors.some(
        (e) => e.field === 'codigoIntegracao' && e.code === 'INTEGRATION_FIELD_FORBIDDEN',
      ),
    ).toBe(true);
  });
